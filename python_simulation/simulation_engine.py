...
class VehicleSimulator:
    ...
    def __init__(self, vehicle_id="V001", vehicle_name="Alpha Truck",
                 log_filepath="data/location_history.csv"):
    ...
    def _ensure_log_file(self) -> None:
        """Create the CSV log file and its parent directory if they don't exist.
        Errors are swallowed so that import-time instantiation never crashes the
        process (critical for Vercel's read-only filesystem)."""
        try:
            ...
            print(f"[VehicleSimulator] Warning: could not init log file: {exc}")

    def _log(self, ts: str) -> None:
        """Append a telemetry row to the CSV. Silently skips on I/O errors."""
        try:
            with open(self.log_filepath, mode="a", newline="") as f:
                csv.writer(f).writerow([
                    ts, self.vehicle_id,
                    round(self.current_lat, 6), round(self.current_lon, 6),
                    round(self.speed, 2), self.heading,
                    self.ignition, self.vibration,
                    round(self.fuel_level, 1), self.battery_voltage,
                    self.engine_rpm, round(self.engine_temp, 1),
                    round(self.driver_score, 1), round(self.trip_distance, 2),
                    self.relay_locked, self.status, self.alert_type,
                    self.theft_risk_score,
                ])
        except (OSError, IOError) as exc:
            print(f"[VehicleSimulator] Warning: could not write log: {exc}")
    ...
    def set_geofence_radius(self, radius) -> None:
        self.geofence_radius = float(radius)
        if self.geofence_zones:
            self.geofence_zones[0]["radius"] = float(radius)

    def add_geofence_zone(self, name, lat, lon, radius) -> None:
        self.geofence_zones.append(
            {"name": name, "lat": float(lat), "lon": float(lon), "radius": float(radius)}
        )

    # ── Internal update helpers ───────────────────────────────────────────────

    def _update_obd(self) -> None:
        if self.ignition:
            ...
            self.fuel_level  = max(0, self.fuel_level - self.speed / 200_000)
    ...
    def _update_driver_score(self) -> None:
        if self.ignition:
    ...
    def _calc_theft_risk(self) -> None:
        risk = 0
        h = datetime.now().hour
        if h >= 23 or h <= 5:
            risk += 25
        ...
        dist = self.calculate_distance(
            self.current_lat, self.current_lon, self.geofence_lat, self.geofence_lon
        )
    ...
    def _state(self, ts: str) -> dict:
        ...
        zones = [
            {
                "name":     z["name"],
                "inside":   self.calculate_distance(
                    self.current_lat, self.current_lon, z["lat"], z["lon"]
                ) <= z["radius"],
        ...
        return {
            ...
            "harsh_braking_events": self.harsh_braking_events,
            ...
            "alert_history":    self.alert_history[-30:],
            "zone_statuses":    zones,
    ...
    def get_history(self, limit: int = 60) -> list:
        ...
        try:
            ...
            print(f"[VehicleSimulator] Warning: could not read history: {exc}")
        return rows[-limit:]
...
class FleetManager:
    ...
    ----------
    data_dir : str
        Directory where per-vehicle CSV logs are written.
        Defaults to ``"data"`` (local dev / Render).
        Pass ``"/tmp/data"`` for Vercel's writable filesystem.
    """
    ...
    def __init__(self, data_dir: str = "data") -> None:
        ...
        for i, (vid, name, dlat, dlon) in enumerate(self.CONFIGS):
            log_path = os.path.join(data_dir, f"fleet_{vid}.csv")
            sim = VehicleSimulator(vid, name, log_path)
    ...
    def set_mode(self, vid: str, mode: str) -> bool:
